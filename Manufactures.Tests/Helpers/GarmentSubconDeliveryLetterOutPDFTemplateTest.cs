...
namespace Manufactures.Tests.Helpers
{
    public class GarmentSubconDeliveryLetterOutPDFTemplateTest
    {
        ...
        public void Generate_Return_Success()
        {
            ...
            var result = GarmentSubconDeliveryLetterOutPDFTemplate.Generate(dto, scDto);
        ...
        public void Generate_Return_SUBCON_JASA_KOMPONEN_Success()
        {
            ...
            var scDto = new GarmentSubconContractDto(new GarmentSubconContract(id, "", "", "", new SupplierId(0), "", "", "", "", "", 0, DateTimeOffset.Now, DateTimeOffset.Now, true, "", new UomId(1), "", "", DateTimeOffset.Now, 0, 0, 0));
            var garmentSubconDLOutItem = new GarmentSubconDeliveryLetterOutItem(id, id, 1, new Domain.Shared.ValueObjects.ProductId(1), "code", "name", "remark", "color", 1, new Domain.Shared.ValueObjects.UomId(1), "unit", new Domain.Shared.ValueObjects.UomId(1), "unit", "fabType", new Guid(), "", "", "22PL0012", "", 1, 1, "");
            var items = new List<GarmentSubconDeliveryLetterOutItemDto>()
            {
                new GarmentSubconDeliveryLetterOutItemDto(garmentSubconDLOutItem)
            };
