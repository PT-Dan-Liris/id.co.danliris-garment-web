...
namespace Manufactures.Application.GarmentPreparings.Queries.GetMonitoringPrepare
{
	public class GetXlsPrepareQueryHandler : IQueryHandler<GetXlsPrepareQuery, MemoryStream>
	{
		...
		public async Task<MemoryStream> Handle(GetXlsPrepareQuery request, CancellationToken cancellationToken)
		{
			...
			//	   roJob = a.RO, 
			//	   buyerCode = a.Buyer, 
			//	   price = Convert.ToDecimal((from aa in sumbasicPrice where aa.RO == a.RO select aa.BasicPrice / aa.Count).FirstOrDefault()), 
			...
			var QueryCuttingDONow = from a in (from data in garmentCuttingInRepository.Query where data.UnitId == request.unit && data.CuttingInDate <= dateTo 
											   select new { 
												   data.RONo, 
												   data.Identity, 
												   data.CuttingInDate, 
            ...
            var QueryAval = from a in (from data in garmentAvalProductRepository.Query where data.AvalDate <= dateTo 
									   select new { 
										   data.Identity, 
										   data.RONo, 
										   data.AvalDate 
                            ...
                            join d in (from data in garmentPreparingRepository.Query where data.UnitId == request.unit 
									   select new { 
										   data.Identity, 
										   data.RONo 
            ...
            var QueryDRPrepare = from a in (from data in garmentDeliveryReturnRepository.Query where data.ReturnDate <= dateTo && data.UnitId == request.unit 
											select new { 
												data.RONo, 
												data.Identity, 
												data.ReturnDate 
            ...
            var querySum = queryNow.GroupBy(x => new { 
				x.a.price, 
				x.b.roasal, 
				x.b.roJob, 
				x.b.article, 
